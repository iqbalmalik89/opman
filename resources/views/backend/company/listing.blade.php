...
<!-- 					        <th class="w-10px pe-2">
 ...
 -->					        <th>Company</th>
					    ...
					    <tbody class="text-gray-600 fw-semibold">
					    	...
					    	<tr>
					    		<td>{{$tenant->id}}</td>
					    		<td>{{$tenant->owner->first_name ?? ''}} {{$tenant->owner->last_name ?? ''}}</td>
					    		<td>{{$tenant->owner->email ?? ''}}</td>
					    		<td> {{$tenant->owner->phone ?? ''}}</td>
					    		<td>{{$tenant->people_count}}</td>
					    		<td>{{$tenant->status}}</td>
					    		<td>
					    			<a href="{{route('secure_login', $tenant->id)}}">Dashboard</a>
						    		@if($tenant->status == 'Active')
						    		<br>
					    			<i class="fa-solid fa-ban text-danger"></i> <a href="{{route('suspend', $tenant->id)}}">Suspend</a>
						    		@elseif($tenant->status == 'Suspended')
						    		<br>
					    			<i class="fa-solid fa-ban text-success"></i> <a href="{{route('unsuspend', $tenant->id)}}">Remove Suspension</a>
						    		@endif
					    		</td>
