...
class SuboperativeStatusChange extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sitemanager:suboperative-status-change';
    ...
    {
        $service = new SuboperativeService();
        $tenants = Tenant::get();
        foreach ($tenants as $key => $tenant) 
        {
            $tenant->run(function () use($service) {

                $service->suboperativeStatusChangeCron();
